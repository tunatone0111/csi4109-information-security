...
fn main() {
    let mut level: HashMap<&str, i8> = HashMap::new();
    level.insert("UNCLASSIFIED", 0);
    level.insert("CONFIDENTIAL", 1);
    level.insert("SECRET", 2);
    level.insert("TOP_SECRET", 3);
    ...
    for perm_raw in perms_raw.lines() {
        ...
        perms.insert(kv[0], *level.get(kv[1]).unwrap());
    }
